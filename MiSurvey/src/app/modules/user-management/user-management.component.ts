...
export class UserManagementComponent implements OnInit {
  ...
  constructor(
    ...
    this.subscription.add(
      this.selectedUser$.pipe(filter(user => user !== null)).subscribe(user => {
        if (user) {
    ...
    this.subscription.add(
      this.store.select(userManagementSelector.selectCurrentUseManagementError).subscribe(error => {
        if (error) {
  ...
  ngOnInit() {
    ...
    this.store.select(userSelector.selectCurrentUser).subscribe(
      id => this.currentUserId = id?.UserID
    );
  }
  ...
  createUser() {
    if (this.addUserForm.valid && this.currentUserId != null) {
      ...
      this.store.dispatch(userManagementActions.createUserRequest({ userData: formData }));
    } else {
  ...
  saveChanges() {
    if (this.editUserFormGroup.valid && this.currentUserId) {
      ...
      this.store.dispatch(userManagementActions.loadUsersRequest());
      this.store.dispatch(userManagementActions.loadUserByIdRequest({ UserID: this.currentUserId }));
    } else {
